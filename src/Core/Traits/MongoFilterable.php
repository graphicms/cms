...
use Gency\Filterable\FilterableException;
use Gency\Filterable\FilterableScope;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\DB;

trait MongoFilterable
{
            ...
            foreach ($rules as $n => $rule) {
                ...
                if (array_key_exists($k, $args)) {
                    ...
                    if(in_array($field, $dates)) {
                     try {
                         $date = Carbon::createFromFormat('U', $args[$k]);
                         $query->$method($field, $date, $root);
                     } catch (\Exception $e) {
                         dd($e);
                     }
                    } else {
